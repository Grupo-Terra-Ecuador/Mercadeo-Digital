...
import { norm, parseNum, normalizeRate, hasMetricValue } from "../format";
import { bestCol } from "../csv/synonyms";
import type { AggregateItem, CsvRow, Dataset, GroupedRow, MetricKey, SynonymKey } from "../types";

export function isTotalRow(row: CsvRow, cols: string[]): boolean {
  return cols
    .map((c) => norm(row[c]))
    .some((v) => v === "total" || v === "totales" || v === "total general" || v === "grand total");
}

export function cleanRows(d: Dataset): CsvRow[] {
  return d.rows.filter((r) => !isTotalRow(r, d.columns));
}

export function metricFrom(d: Dataset, key: MetricKey): number {
...
export function preferredMetric(ds: Dataset[], key: MetricKey): { value: number; dataset: Dataset | null } {
  for (const d of ds) {
...
export function firstPositiveMetric(ds: Dataset[], key: MetricKey): number {
...
export function aggregate(ds: Dataset[], labelKey: SynonymKey, valueKey: MetricKey, limit = 50): AggregateItem[] {
...
export function weightedRate(ds: Dataset[], rateKey: SynonymKey, weightKey: SynonymKey = "sessions"): number {
  let num = 0;
  let den = 0;
  ds.forEach((d) => {
...
export function bounceFromDatasets(ds: Dataset[]): { value: number; derived: boolean } {
...
export function groupRows(ds: Dataset[], labelKey: SynonymKey): GroupedRow[] {
  ...
  ds.forEach((d) => {
    ...
    cleanRows(d).forEach((r) => {
      ...
      (["users", "newUsers", "returningUsers", "sessions", "views", "clicks", "impressions"] as const).forEach((k) => {
        const col = cols[k];
        if (col && hasMetricValue(r[col])) {
          o[k] += parseNum(r[col]);
          (o as unknown as Record<string, boolean>)[`${k}Available`] = true;
        }
      });
      let br = 0;
      ...
      const w =
        cols.sessions && hasMetricValue(r[cols.sessions])
...
export function bestDataset(ds: Dataset[], labelOptions: SynonymKey[] = [], metricOptions: SynonymKey[] = []): Dataset | null {
