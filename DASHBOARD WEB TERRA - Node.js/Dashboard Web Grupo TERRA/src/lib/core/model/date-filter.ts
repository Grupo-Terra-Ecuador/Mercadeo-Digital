...
//
// Estas funciones reciben `settings` explicito en vez de leer un estado global, para que
// sean puras y testeables de forma aislada.
import { parseReportDateToken, dateISO } from "../format";
import { findRowDateCol } from "../csv/parse";
import type { Dataset, Settings } from "../types";

export function isDateFilterActive(settings: Settings): boolean {
  return !!(settings.dateFrom || settings.dateTo);
}

export function selectedDateRange(settings: Settings): { from: Date | null; to: Date | null } {
  return {
...
export function rangesOverlap(a1: Date | null, a2: Date | null, b1: Date | null, b2: Date | null): boolean {
  if (!a1 || !a2 || !b1 || !b2) return true;
  return a1 <= b2 && b1 <= a2;
}

export function filterDatasetByDate(d: Dataset, settings: Settings): Dataset {
