import { esc, fmt, fmtCompact, fmtDuration, pct, parseNum, hasMetricValue, rateStatus } from "../../core/format";
...
const SUMMARY_BAR_COLOR: Record<string, string> = { blue: "bg-blue", purple: "bg-purple", green: "bg-green" };

interface SummaryInsightOpts {
  label: string;
  value: string;
  cls?: "blue" | "purple" | "green" | "";
  rows?: { label: string; value?: string; html?: string }[];
  cities?: { label: string; value: number }[];
  tipText?: string;
}

export function summaryInsight(id: string, { label, value, cls = "", rows = [], cities = [], tipText = "" }: SummaryInsightOpts): void {
  const el = document.getElementById(id);
  if (!el) return;
  const bar = SUMMARY_BAR_COLOR[cls] || "bg-orange";
  el.innerHTML = `<div class="relative min-h-[160px] overflow-hidden rounded-terra border border-border bg-surface px-[18px] py-4"><span class="absolute inset-x-0 top-0 h-[3px] ${bar}"></span>${
    tipText
      ...
      : ""
  }<div class="mb-2 text-[10px] font-black uppercase tracking-wide text-muted-2">${esc(label)}</div><div class="overflow-wrap-anywhere text-[23px] font-black leading-[1.15] text-text">${esc(
    value || "No disponible"
  )}</div><div class="mt-3 grid gap-[5px]">${rows
    .map(
      (r) =>
        `<div class="flex justify-between gap-2.5 border-t border-border-2/55 pt-1.5 text-[11px] text-muted"><span>${esc(r.label)}</span><b class="text-right text-text">${r.html ?? esc(r.value)}</b></div>`
    )
    .join("")}${
    cities.length
      ? `<div class="mt-2 grid gap-1">${cities
          .map((c, i) => `<div class="flex justify-between gap-2 rounded-lg bg-white/[0.035] px-[7px] py-[5px] text-[10px] text-muted"><span>${i + 1}. ${esc(c.label)}</span><b class="text-text">${fmt(c.value)}</b></div>`)
          .join("")}</div>`
      : ""
  }</div></div>`;
}

export function renderGA4Snapshot(): void {
  const el = document.getElementById("ga4Snapshot");
  if (!el) return;
  const { datasets } = dashboardStoreApi.getState();
  const src = datasets.find((d) => d.name === GA4_TOTALS_DATASET_NAME);
  if (!src) {
    el.innerHTML = `<div class="rounded-[13px] border border-dashed border-border-2 bg-white/[0.015] p-[18px] text-center text-xs text-muted-2">Conecta con Google para ver esta vista (requiere la consulta agregada del periodo, no disponible con CSV manual).</div>`;
    return;
  }
  ...
  const items = [
    { label: "Usuarios activos", value: active !== null ? fmtCompact(active) : "-", tip: active !== null ? `${fmt(active)} usuarios activos exactos en el periodo.` : "Sin datos disponibles." },
    { label: "Usuarios nuevos", value: news !== null ? fmtCompact(news) : "-", tip: news !== null ? `${fmt(news)} usuarios nuevos exactos en el periodo.` : "Sin datos disponibles." },
    {
    ...
    },
    {
      ...
      tip: events !== null ? `${fmt(events)} eventos exactos (vistas de pagina, clics, desplazamientos y cualquier otra interaccion registrada) en el periodo.` : "Sin datos disponibles.",
    },
  ];
  el.innerHTML = items
    .map(
      (i, idx, arr) =>
        `<div class="flex-[1_1_150px] cursor-help border-r border-border px-5 py-4 max-[720px]:flex-[1_1_45%] max-[720px]:border-b max-[720px]:border-r-0${idx === arr.length - 1 ? " border-r-0" : ""}" data-tip="${esc(i.tip)}"><div class="mb-2 text-xs text-muted-2">${esc(i.label)}</div><div class="text-[26px] font-extrabold leading-[1.2] text-text">${i.value}</div></div>`
    )
    .join("");
}

export function renderSummary(m: DashboardModel): void {
  renderGA4Snapshot();
  ...
  if (kpisEl) {
    kpisEl.innerHTML =
      ...
      kpi("Sesiones", fmt(m.sessions), "Adquisicion de trafico", "", 'Una sesion es un periodo de actividad continua de una persona en el sitio (equivalente a "una visita"). Un mismo usuario puede generar varias sesiones en dias distintos del periodo, por eso el numero de sesiones suele ser mayor al numero de usuarios.') +
      kpi(
        "Tasa de rebote",
  ...
  summaryInsight("summaryTech", {
    ...
    cls: "purple",
    tipText: "Resumen del entorno tecnologico mas relevante detectado en la audiencia: categoria de dispositivo, sistema operativo, marca o modelo cuando este disponible y navegador principal.",
    rows: [
...
export function renderTrafficTrend(): void {
  const { datasets } = dashboardStoreApi.getState();
