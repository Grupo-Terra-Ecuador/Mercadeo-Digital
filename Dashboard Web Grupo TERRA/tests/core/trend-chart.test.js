...
import { extractDailySeries, fullDateRangeOfDatasets } from '../../src/charts/trend-chart.js';

function toDataset(name, columns, rows) {
...
describe('extractDailySeries', () => {
  it('agrupa por fecha y por etiqueta, y limita a los `limit` canales con mayor volumen', () => {
    const ds = [
      toDataset('daily.csv', ['Fecha', 'Grupo de canales principal de la sesion', 'Sesiones'], [
  ...
  it('devuelve null si ningun dataset tiene fecha por fila', () => {
    const ds = [
      toDataset('sin-fecha.csv', ['Grupo de canales principal de la sesion', 'Sesiones'], [['Organic Search', '100']]),
    ];
...
describe('fullDateRangeOfDatasets', () => {
  it('calcula el rango min/max entre todas las filas con fecha', () => {
    const ds = [
      toDataset('daily.csv', ['Fecha', 'Sesiones'], [
        ['20240105', '1'],
        ['20240120', '2'],
        ['20240110', '3'],
      ]),
