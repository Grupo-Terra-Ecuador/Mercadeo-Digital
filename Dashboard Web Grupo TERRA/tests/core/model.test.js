...
import { parseCSV } from '../../src/core/csv/parse.js';
import { classify } from '../../src/core/csv/classify.js';
import { buildModel } from '../../src/core/model/build-model.js';
import { isDateFilterActive, filterDatasetByDate } from '../../src/core/model/date-filter.js';

function toDataset(name, text) {
...
const NO_FILTER_SETTINGS = { dateFrom: '', dateTo: '', dateMode: 'safe' };

describe('buildModel', () => {
  it('agrega usuarios totales, nuevos y estima recurrentes cuando el CSV no los trae', () => {
    const datasets = [
      toDataset('adquisicion_de_trafico.csv', TRAFFIC_CSV),
      toDataset('adquisicion_de_usuarios.csv', USERS_CSV),
    ];
  ...
  it('excluye las filas "Total" del calculo de agregados', () => {
    ...
    // 1200 + 300 = 1500, no 1200+300+1500 (que incluiria la fila Total).
