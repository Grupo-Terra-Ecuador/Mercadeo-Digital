...
import { norm, parseNum, parseReportDateToken, dateLabel } from '../format.js';
import { metricKeys, labelKeys, synonyms } from './synonyms.js';

// Construidos con String.fromCharCode (no literales) por la misma razon que en format.js:
...
export function splitRows(text, d) {
...
export function headerScore(row, next = []) {
  ...
  let nums = 0;
  next.slice(0, 6).forEach((r) => r.forEach((c) => {
    if (parseNum(c) !== 0) nums++;
  }));
  return score + Math.min(12, nums);
}

export function uniqueColumns(cols) {
  const seen = {};
  return cols.map((c, i) => {
...
export function findRowDateCol(cols) {
  return (
    (cols || []).find((c) => {
      const n = norm(c);
