...
//
// A diferencia del original, buildModel(datasets, settings) recibe sus datos de entrada
// como parametros en vez de leer un objeto `state` global: no muta nada externo, solo
// devuelve el modelo. El llamador (state.js) es responsable de guardar el resultado.
...
import { isDateFilterActive, filterDatasetByDate } from './date-filter.js';
import { GA4_TOTALS_DATASET_NAME } from './dataset-names.js';
...
export function datasetPriority(type) {
  return DATASET_PRIORITY[type] ?? 9;
}

export function summarizeDateFilter(allDatasets, base, settings) {
  const ranges = allDatasets.filter((d) => d.reportStart && d.reportEnd).map((d) => ({ start: d.reportStart, end: d.reportEnd }));
  let min = null;
  let max = null;
  ranges.forEach((r) => {
...
export function buildModel(datasets, settings) {
  const base = datasets
  ...
  const totalsPrimary = usersDs.find((d) => d.name === GA4_TOTALS_DATASET_NAME);
  const totalUsers =
  ...
  if (userChannelPrimary) {
    ...
    userChannelRows.forEach((u) => {
      const current = channelMap.get(u.label) || {
        label: u.label,
        users: 0,
        newUsers: 0,
        returningUsers: 0,
