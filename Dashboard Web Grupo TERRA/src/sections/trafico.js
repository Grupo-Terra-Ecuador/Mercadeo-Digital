import { esc, fmt, pct, norm } from '../core/format.js';
...
import { tip } from '../ui/render-helpers.js';
...
export function channelExplanation(label) {
  const n = norm(label);
...
export function renderTraffic(m) {
  ...
  const rows = m.channels
    .map(
      (x) =>
        `<tr><td>${tip(x.label, channelExplanation(x.label))}</td><td class="num">${x.sessionsAvailable ? fmt(x.sessions) : '-'}</td><td class="num">${x.usersAvailable ? fmt(x.users) : '-'}</td><td class="num">${x.bounceAvailable ? pct(x.bounceRate) : '-'}</td><td class="num">${pct(metricTotal(data) ? (x.sessions || x.users) / metricTotal(data) : 0)}</td></tr>`
    )
    .join('');
  document.getElementById('channelTable').innerHTML = rows
    ? `<table><thead><tr><th>${tip('Canal', 'Clasificacion automatica de GA4 que agrupa el origen y el medio de cada visita, por ejemplo: busqueda organica, acceso directo, campanas pagadas, redes sociales o referencias desde otros sitios.')}</th><th class="num">${tip('Sesiones', 'Cantidad de visitas iniciadas desde este canal durante el periodo. Una misma persona puede generar varias sesiones, por lo que esta cifra puede ser mayor que Usuarios.')}</th><th class="num">${tip('Usuarios', 'Personas distintas asociadas a este canal segun el informe cargado. La suma por canal puede no coincidir exactamente con el total general por deduplicacion y por el alcance de la dimension utilizada.')}</th><th class="num">${tip('Rebote', 'Porcentaje de sesiones de este canal que no fueron sesiones con interaccion. Se muestra solo cuando el CSV incluye Tasa de rebote o Tasa de interaccion compatible.')}</th><th class="num">${tip('Participacion', 'Porcentaje directo del volumen clasificado que aporta este canal: valor del canal / total de la tabla x 100. Se priorizan Sesiones y, si no existen, se usan Usuarios. No representa ventas ni conversiones.')}</th></tr></thead><tbody>${rows}</tbody></table>`
    : '<table><tbody><tr><td>Sin canales detectados.</td></tr></tbody></table>';
  ...
  document.getElementById('channelNote').innerHTML = top
...
export function renderSessionsTrend() {
...
export function renderBounceTrend() {
  const src = state.datasets.filter((d) => d.type === 'trafico');
