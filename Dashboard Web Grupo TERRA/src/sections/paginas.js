...
export function renderPages(m) {
  ...
  const rows = visibleRows
    .map(
      (x) =>
        `<tr><td>${esc(x.label)}</td><td class="num">${x.viewsAvailable ? fmt(x.views) : '-'}</td><td class="num">${x.usersAvailable ? fmt(x.users) : '-'}</td><td class="num">${x.bounceAvailable ? pct(x.bounceRate) : '-'}</td><td class="num">${pct(total ? (x.views || x.users) / total : 0)}</td></tr>`
    )
    .join('');
  const pageTable = document.getElementById('pageTable');
  if (pageTable) pageTable.className = 'tablewrap limit-10';
  document.getElementById('pageTable').innerHTML = rows
    ? `<table><thead><tr><th>${tip('Pagina', 'Pagina, ruta, pantalla o pagina de destino identificada en el informe cargado. El texto mostrado depende de la dimension que fue exportada desde GA4.')}</th><th class="num">${tip('Vistas', 'Numero total de veces que la pagina fue vista. Incluye vistas repetidas de una misma persona durante una o varias sesiones.')}</th><th class="num">${tip('Usuarios', 'Personas distintas que vieron esta pagina. Solo se muestra cuando el CSV contiene una metrica de usuarios compatible con la dimension de pagina.')}</th><th class="num">${tip('Rebote', 'Porcentaje de sesiones asociadas a esta pagina que no tuvieron interaccion. Solo se muestra si el informe incluye Rebote o Interaccion y una base de sesiones compatible.')}</th><th class="num">${tip('Participacion', 'Porcentaje directo que aporta la fila al total procesado: Vistas de la pagina / Vistas totales x 100; si no hay Vistas, se usan Usuarios. No representa conversiones ni ventas.')}</th></tr></thead><tbody>${rows}</tbody></table>`
    : '<table><tbody><tr><td>Sin paginas detectadas.</td></tr></tbody></table>';
